...
JOB='''module load picard BWA && bwa mem /ysm-gpfs/datasets/genomes/Homo_sapiens/UCSC/hg38/Sequence/BWAIndex/genome.fa %(fq)s | \
samtools sort -O bam -o %(obn)s.sorted.bam && \
...
for fq in glob.glob(os.path.join(inputdir, "*.fq")):
